...
void coo_to_csr(const std::vector<std::pair<int, int>>& coo_entries,
  ...
  int nnz = coo_entries.size(); // Number of non-zero entries
  tree_idx.resize(nnz);
  tree_ptr.resize(numRows + 1, 0);

  // Populate columns array and count the number of entries per row
  for (int i = 0; i < nnz; ++i) {
    tree_idx[i] = coo_entries[i].second;
    tree_ptr[coo_entries[i].first + 1]++;
  }
  ...
  // Temporary array to store the current insertion point in each row
  std::vector<int> current_pos = tree_ptr;

  // Fill in the values and columns array
  for (int i = 0; i < nnz; ++i) {
    int row = coo_entries[i].first;
    int col = coo_entries[i].second;
  ...
  //Sort the entries
  for(int i = 0; i < numRows; i++){
    std::sort(tree_idx.begin() + tree_ptr[i], tree_idx.begin() + tree_ptr[i + 1]);
  }
}
